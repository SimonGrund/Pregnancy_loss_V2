...
d = filter(d, patient_type %in% c(1,2))
table(d$group)
...
g
quartz(type = 'pdf', file = "Results/Figures/Rate_of_pregnancy_and_live_birth.pdf", width = 5, height = 6)
g
dev.off()

out = bind_rows(achieved_preg, live_birth)

openxlsx::write.xlsx(out, "Results/Tables/Achieved_preg_and_live_births.xlsx")
